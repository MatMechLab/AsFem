#ifndef ASFEM_RANKTWO_TENSOR
#define ASFEM_RANKTWO_TENSOR
...
public:
    RankTwoTensor(double val=0.0);
    RankTwoTensor(InitMethod method);

    // this is quite helpful for deformation gradient calculation
    RankTwoTensor(const Eigen::Vector3d &r1,
                  const Eigen::Vector3d &r2);// for 2d case
    RankTwoTensor(const Eigen::Vector3d &r1,
                  const Eigen::Vector3d &r2,
                  const Eigen::Vector3d &r3);// for 3d case
    ...
    inline Eigen::Vector3d IthRow(const int &i)const{
        Eigen::Vector3d temp;
        temp.coeffRef(0)=(*this)(i,1);
        temp.coeffRef(1)=(*this)(i,2);
        temp.coeffRef(2)=(*this)(i,3);
        return temp;
    }
    inline Eigen::Vector3d IthCol(const int &i)const{
        Eigen::Vector3d temp;
        temp.coeffRef(0)=(*this)(1,i);
        temp.coeffRef(1)=(*this)(2,i);
        temp.coeffRef(2)=(*this)(3,i);
        return temp;
    }
    //***********************
    //*** for = operator
    ...
    friend RankTwoTensor CrossDot(const Eigen::Vector3d &a,const Eigen::Vector3d &b);
    ...
    friend Eigen::Vector3d operator*(const Eigen::Vector3d &lhs,const RankTwoTensor &a);

    inline Eigen::Vector3d operator*(const Eigen::Vector3d &a) const{
        Eigen::Vector3d temp;
        for(int i=1;i<=_N;++i){
            temp.coeffRef(i-1)=0.0;
            for(int j=1;j<=_N;++j){
                temp.coeffRef(i-1)+=(*this)(i,j)*a.coeff(j-1);
            }
        }
    ...
    inline void VectorCrossDot(const Eigen::Vector3d &a,const Eigen::Vector3d &b){
        for(int i=1;i<=_N;++i){
            for(int j=1;j<=_N;++j){
                (*this)(i,j)=a.coeff(i-1)*b.coeff(j-1);
            }
        }
    }
    ...
    inline RankTwoTensor Inverse() const{
        double J=Det();
        if(abs(J)<1.0e-15){
            cout<<"*** Error: inverse failed for a singular rank-2 tensor!!!  ***"<<endl;
            Msg_AsFem_Exit();
        }
    ...
    void SetFromGradU(const Eigen::Vector3d &gradUx,const Eigen::Vector3d &gradUy);
    void SetFromGradU(const Eigen::Vector3d &gradUx,const Eigen::Vector3d &gradUy,const Eigen::Vector3d &gradUz);
