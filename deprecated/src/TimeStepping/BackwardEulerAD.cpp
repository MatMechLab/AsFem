...
void TimeStepping::BackwardEulerAD(Mesh &mesh,DofHandler &dofHandler,
    ...
    for(_CurrentTime=0.0;_CurrentTime<=_TotalTime;){
        ...
        if(nonLinearSolver.Solve(mesh,dofHandler,solutionSystem,equationSystem,
            ...
            if(!IsConvergent){
                ...
                cout<<"*** Minimal dt is reached, transient analysis(BE) failed!!!***"<<endl;
        ...
        if(nonLinearSolver.GetCurrentIters()<=_OptIters){
            ...
            if(feCtrlInfo._dt<_MinDt) feCtrlInfo._dt=_MinDt;
        }
        else{
            feCtrlInfo._dt*=_CutbackFactor;
            if(feCtrlInfo._dt<_MinDt) feCtrlInfo._dt=_MinDt;
