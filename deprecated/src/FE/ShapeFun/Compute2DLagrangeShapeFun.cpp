...
void ShapeFun::Compute2DLagrangeShapeFun(const double &xi,const double &eta,const Nodes &nodes)
{
    ...
    // if(GetDim()==2)
    // {
    //     _DetJac=_dxdxi*_dydeta-_dxdeta*_dydxi;
    // }
    // else
    {
        // "2D" element can also have "3D" properties
    ...
    if(abs(_DetJac)<1.0e-15){
        Msg_ShapeFun_Lagrange2DElmtSingular();
        Msg_AsFem_Exit();
    }
    ...
    for(int i=1;i<=GetShapeFunNums();i++){
       ...
       _shape_grad[i-1].setZero();
        _shape_grad[i-1].coeffRef(0)=(*this)(i,1);
        _shape_grad[i-1].coeffRef(1)=(*this)(i,2);
    }
}