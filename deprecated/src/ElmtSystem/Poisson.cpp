...
void ElmtSystem::Poisson(const int &isw,const int &nDim,const int &nNodes,const double &JxW,
    ...
    if(isw==3||isw==6){
        for(int i=1;i<=nNodes;++i){
            rhs.coeffRef(i-1)+=MaterialValues[0]*(gpGradU[0]*shp.shape_grad(i))*JxW
                              +MaterialValues[2]*shp.shape_value(i)*JxW;
            if(isw==6){
                for(int j=1;j<=nNodes;++j){
                    
                    K.coeffRef(i-1,j-1)+=
                      MaterialValues[1]*shp.shape_value(j)*(gpGradU[0]*shp.shape_grad(i))*ctan[0]*JxW
                     +MaterialValues[0]*(shp.shape_grad(j)*shp.shape_grad(i))*ctan[0]*JxW
                     +MaterialValues[3]*shp.shape_value(j)*shp.shape_value(i)*ctan[0]*JxW;
                }
            }
        }
    }
