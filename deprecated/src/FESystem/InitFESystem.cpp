...
void FESystem::InitFESystem(Mesh &mesh,
    ...
    for(int i=0;i<dofHandler.GetDofsNumPerNode();++i){
        _gpU.push_back(0.0);
        _gpV.push_back(0.0);
        _gpGradU.push_back(Eigen::Vector3d::Zero());
        _gpGradV.push_back(Eigen::Vector3d::Zero());
    }

    
    //_nHist=solutionSystem.GetHistNumPerElmt();
    //_nProj=solutionSystem.GetProjNumPerNode();
    ...
    if(qpBlockInfo._SetFromInput){
        fe.SetOrder(qpBlockInfo._nQpOrder);
    }
    else{
        fe.SetOrder(mesh.GetMeshOrder()+1);
    }
    fe.InitFE(mesh,qpBlockInfo._QpType);

    
    _localK=Eigen::MatrixXd(dofHandler.GetMaxDofsNumPerElmt(),dofHandler.GetMaxDofsNumPerElmt());
    _localR=Eigen::VectorXd(dofHandler.GetMaxDofsNumPerElmt());
