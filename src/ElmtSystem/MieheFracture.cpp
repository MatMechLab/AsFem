...
void ElmtSystem::MieheFracture(const int &isw,const int &nDim,const int &nNodes,
        ...
        for(int i=1;i<=nNodes;++i){
            if(nDim==2){
                ...
                rhs(3*i  )+=viscosity*gpV[nDim]*shp.shape_value(i)
                           +2*(gpU[nDim]-1)*H*shp.shape_value(i)
                           +(Gc/L)*gpU[nDim]*shp.shape_value(i)
                           +Gc*L*(gpGradU[nDim]*shp.shape_grad(i));
            }
            else if(nDim==3){
                // R_ux
            ...
            if(isw==6){
                for(int j=1;j<=nNodes;++j){
                    if(nDim==2){
                        ...
                        // Kd,d
                        K(3*i  ,3*j  )+=2*H*shp.shape_value(j)*shp.shape_value(i)*ctan[0]
                                       +(Gc/L)*shp.shape_value(j)*shp.shape_value(i)*ctan[0]
                                       +Gc*L*(shp.shape_grad(j)*shp.shape_grad(i))*ctan[0];
                        ...
                        // Kc,ux
                        K(3*i  ,3*j-2)+=2*(gpU[nDim]-1)*valx*shp.shape_value(i)*ctan[0];
                        // Kc,uy
                        K(3*i  ,3*j-1)+=2*(gpU[nDim]-1)*valy*shp.shape_value(i)*ctan[0];
                        ...
                        // Kd,d
                        K(4*i  ,4*j  )+=2*H*shp.shape_value(j)*shp.shape_value(i)*ctan[0]
                                       +(Gc/L)*shp.shape_value(j)*shp.shape_value(i)*ctan[0]
                                       +Gc*L*(shp.shape_grad(j)*shp.shape_grad(i))*ctan[0];
                        ...
                        // Kd,ux
                        K(4*i  ,4*j-3)+=2*(gpU[nDim]-1)*valx*shp.shape_value(i)*ctan[0];
                        // Kd,uy
                        K(4*i  ,4*j-2)+=2*(gpU[nDim]-1)*valy*shp.shape_value(i)*ctan[0];
                        // Kd,uz
                        K(4*i  ,4*j-1)+=2*(gpU[nDim]-1)*valz*shp.shape_value(i)*ctan[0];
                    }
                }
            }
        }
