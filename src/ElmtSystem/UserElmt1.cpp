...
void ElmtSystem::UserElmt1(const int &isw,const int &nDim,const int &nNodes,
    ...
    if(isw==3||isw==6){
        for(int i=1;i<=nNodes;++i){
            rhs(i)+=(1+gpU[0]*gpU[0])*(gpGradU[0]*shp.shape_grad(i))
                   +ScalarMaterials[2]*shp.shape_value(i);
            if(isw==6){
                for(int j=1;j<=nNodes;++j){
                    K(i,j)+=2*gpU[0]*shp.shape_value(j)*(gpGradU[0]*shp.shape_grad(i))*ctan[0]
                           +(1+gpU[0]*gpU[0])*(shp.shape_grad(j)*shp.shape_grad(i))*ctan[0]
