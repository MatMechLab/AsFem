...
void MateSystem::BordenLinearElasticMaterial(const int &nDim,const double &t,const double &dt,
    ...
    double d=gpU[nDim];
    if(d<0.0) d=0.0;// damaged
    if(d>1.0) d=1.0;// undamaged
    const double k=1.0e-5; // to avoid the zero stiffness matrix

    g=d*d;dg=2.0*d;d2g=2.0;
    ...
    _Rank2Materials[2]=StressPos*dg;//dStress/dD
    ...
    SignPos=0.0;
    if(TrPos>0.0) SignPos=1.0;
    SignNeg=0.0;
    if(TrNeg>0.0) SignNeg=1.0;
    _Rank4Materials[0]=(I.CrossDot(I)*lambda*SignPos+ProjPos*2*mu)*(g+k)
