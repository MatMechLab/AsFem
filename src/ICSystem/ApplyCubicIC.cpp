...
void ICSystem::ApplyCubicIC(const int &DofIndex, const vector<double> &Parameters, const vector<string> &DomainList,
                            const Mesh &mesh, const DofHandler &dofHandler, Vec &U) {
    if(Parameters.size()<7){
        MessagePrinter::PrintErrorTxt("for constant IC, you need at least four parameter, params='x0 y0 z0 dx dy dz value' is expected in [ics]");
    ...
    double x0,y0,z0,dx,dy,dz,value;
    double x,y,z;
    x0=Parameters[0];y0=Parameters[1];z0=Parameters[2];
    dx=Parameters[3];dy=Parameters[4];dz=Parameters[5];
    value=Parameters[6];
