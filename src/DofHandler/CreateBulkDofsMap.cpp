...
void BulkDofHandler::createBulkDofsMap(const FECell &t_fecell,const ElmtSystem &t_elmtSystem){
    if(t_fecell.getFECellMaxDim()|| t_elmtSystem.getBulkElmtBlocksNum()){}
    // m_bulkelmts=t_fecell.getFECellBulkElmtsNum();
    // m_nodes=t_fecell.getFECellNodesNum();
    // m_total_dofs=m_nodes*m_maxdofs_pernode;// maxdofs_pernode dependes on the dofs name 
    //                                        // one defined in the input file
    // m_maxdofs_perelmt=m_maxdofs_pernode*t_fecell.getFECellNodesNumPerBulkElmt();
    ...
    //         for(int e=1;e<=t_fecell.getFECellBulkElmtsNumViaPhyName(name);e++){
