...
//+++ Author : Yang Bai
//+++ Date   : 2024.02.03
//+++ Purpose: the 1D edge2 lagrange FE cell generator
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

#include "FECell/Lagrange1DEdge2MeshCellGenerator.h"
#include "FECell/SingleMeshCell.h"

Lagrange1DEdge2MeshCellGenerator::Lagrange1DEdge2MeshCellGenerator(){
...
//*********************************************************
bool Lagrange1DEdge2MeshCellGenerator::generateFECell(FECellData &t_celldata){
    int rank;
    t_celldata.MaxDim=1;
    t_celldata.MinDim=0;
    ...
    if(rank==0){
        ...
        t_celldata.BulkMeshTypeName="edge2";
        ...
        t_celldata.NodesNum=t_celldata.Nx+1;
        t_celldata.NodesNumPerBulkElmt=2;
        t_celldata.NodesNumPerSurfElmt=0;
        t_celldata.NodesNumPerLineElmt=0;

        double dx,dy,dz;
        dx=(t_celldata.Xmax-t_celldata.Xmin)/(static_cast<double>(t_celldata.NodesNum)-1.0);
        dy=(t_celldata.Ymax-t_celldata.Ymin)/(static_cast<double>(t_celldata.NodesNum)-1.0);
        dz=(t_celldata.Zmax-t_celldata.Zmin)/(static_cast<double>(t_celldata.NodesNum)-1.0);
            
        t_celldata.BulkElmtVTKCellType=3;
        t_celldata.BulkElmtMeshType=MeshType::EDGE2;
        ...
        int i1,i2;
        for(int e=0;e<t_celldata.BulkElmtsNum;e++){
            t_celldata.PhyName2BulkFECellIDMap_Global["alldomain"].push_back(e+1);
            t_celldata.PhyID2BulkFECellIDMap_Global[0].push_back(e+1);

            i1=e*t_celldata.MeshOrder+1;
            i2=i1+1;
            ...
            t_celldata.MeshCell_Total[e].ElmtNodeCoords.resize(2);
            t_celldata.MeshCell_Total[e].ElmtNodeCoords(1,1)=t_celldata.NodeCoords_Global[(i1-1)*3+1-1];
            t_celldata.MeshCell_Total[e].ElmtNodeCoords(1,2)=t_celldata.NodeCoords_Global[(i1-1)*3+2-1];
            t_celldata.MeshCell_Total[e].ElmtNodeCoords(1,3)=t_celldata.NodeCoords_Global[(i1-1)*3+3-1];
            //
            t_celldata.MeshCell_Total[e].ElmtNodeCoords(2,1)=t_celldata.NodeCoords_Global[(i2-1)*3+1-1];
            t_celldata.MeshCell_Total[e].ElmtNodeCoords(2,2)=t_celldata.NodeCoords_Global[(i2-1)*3+2-1];
            t_celldata.MeshCell_Total[e].ElmtNodeCoords(2,3)=t_celldata.NodeCoords_Global[(i2-1)*3+3-1];

            t_celldata.MeshCell_Total[e].ElmtNodeCoords0=t_celldata.MeshCell_Total[e].ElmtNodeCoords;
            ...
            if(i2==t_celldata.NodesNum){
                ...
                rightconn[1-1].ElmtConn.push_back(i2);

                rightconn[1-1].ElmtNodeCoords0.resize(1);
                rightconn[1-1].ElmtNodeCoords0(1,1)=t_celldata.NodeCoords_Global[(i2-1)*3+1-1];
                rightconn[1-1].ElmtNodeCoords0(1,2)=t_celldata.NodeCoords_Global[(i2-1)*3+2-1];
                rightconn[1-1].ElmtNodeCoords0(1,3)=t_celldata.NodeCoords_Global[(i2-1)*3+3-1];
