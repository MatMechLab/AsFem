...
bool Lagrange1DEdge4MeshCellGenerator::generateFECell(FECellData &t_celldata){
    ...
    if(rank==0){
        ...
        for(int i=1;i<=t_celldata.NodesNum;i++){
            t_celldata.NodeCoords_Global[(i-1)*3+1-1]=t_celldata.Xmin+(i-1)*dx;
            t_celldata.NodeCoords_Global[(i-1)*3+2-1]=t_celldata.Ymin+(i-1)*dy;
            t_celldata.NodeCoords_Global[(i-1)*3+3-1]=t_celldata.Zmin+(i-1)*dz;
        ...
        for(int e=1;e<=t_celldata.BulkElmtsNum;e++){
            ...
            t_celldata.MeshCell_Total[e-1].Volume=3.0*dx;
            ...
            if(i4==t_celldata.NodesNum){
                ...
                rightconn[1-1].ElmtNodeCoords0(1,1)=t_celldata.NodeCoords_Global[(i4-1)*3+1-1];
                rightconn[1-1].ElmtNodeCoords0(1,2)=t_celldata.NodeCoords_Global[(i4-1)*3+2-1];
                rightconn[1-1].ElmtNodeCoords0(1,3)=t_celldata.NodeCoords_Global[(i4-1)*3+3-1];
